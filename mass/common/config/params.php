<?php
return [
    'adminEmail' => '18611348367@163.com',
    'supportEmail' => '18611348367@163.com',
    'user.passwordResetTokenExpire' => 3600,
    'site-keywords'=>'大众广告',
    'site-desc'=>'大众广告管理后台',
    'appkey'=>'23437908',//淘宝APPKEY
    'secretKey'=>'02c128a923720e3adea4af39b3e9a44c',//淘宝secretkey
    'photoUrl'=>'http://images.51guanggao.cc/photo/',
    'uploadUrl'=>'http://images.51guanggao.cc/',
    'baiduMapAK'=>'UbIBo7mo9wDuDz20VoEiAe8G',//百度地图ak
    'wxpayNotifyUrl'=>'http://api.51guanggao.cc/site/wxpay-notify',//微信支付异步通知
    'alipayNotifyUrl'=>'http://api.51guanggao.cc/site/alipay-notify',//微信支付异步通知
    'alipayConfig'=>[
        //应用ID,您的APPID。
        'app_id' => "2017040306539395",//正式账号
        'pid'=>'2088421965382161',
//         'app_id' => '2016072900115783',//沙箱账号
        
        //商户私钥，您的原始格式RSA私钥
        'merchant_private_key' => "",
        //异步通知地址
        'notify_url' => "http://api.51guanggao.cc/site/alipay-notify",
//         'notify_url' => "http://api.eoil.mi2you.com/site/alipay-notify",
        //同步跳转
        'return_url' => "http://api.51guanggao.cc/site/alipay-return",
        'base_path'=>"http://api.51guanggao.cc",
        //编码格式
        'charset' => "UTF-8",
        
        //支付宝网关
        'gatewayUrl' => "https://openapi.alipay.com/gateway.do",//正式地址
//         'gatewayUrl' => "https://openapi.alipaydev.com/gateway.do",//沙箱地址
        
        //支付宝公钥
        'alipay_public_key' => "",
        
    ]
];
