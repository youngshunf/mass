...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if($inputObj->GetTrade_type() =='APP'){
		    ...
		    $res = WxPayResults::InitFromArray($prePayParams,true);
		    $res->SetSign();
	...
	/**
	 *
	 * 企业付款，WxPayRefund中out_trade_no、transaction_id至少填一个且
	 * out_refund_no、total_fee、refund_fee、op_user_id为必填参数
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
	...
	public static function enterprisePay($inputObj, $timeOut = 6)
	{
	    ...
	    if(!$inputObj->IsPartner_trade_noSet() ) {
	        throw new WxPayException(" 企业付款接口中，o缺少必填参数partner_trade_no！");
	    ...
	    $result = WxPayResults::InitNoCheckSign($response);
	    self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
